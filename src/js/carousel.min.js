var lWrapper = "carousel-list-wrapper",
...
function setListAndItems(listDest, itemDest, items) {
  this.setMenu(listDest, items), this.setItems(itemDest, items);
}

function setItems(destEl, items, parent) {
  if (Array.isArray(items)) {
    for (var selectedExists, i = 0; items.length > i; i++)
      items[i].hasOwnProperty("uid") || getUID(items[i]);
    if (
      (destEl instanceof HTMLCollection && (destEl = destEl.item(0)),
      !(destEl instanceof HTMLElement))
    )
      return console.warn(
        "Warning: please provide setItems with a valid destination element!"
      );
    destEl.childNodes.forEach(function (node) {
      if (node.className)
        return !!node.className.includes(selected) && (selectedExists = !0);
    }),
      items.forEach(function (item, idx) {
        void 0 === parent && (parent = !0);
        var el = document.createElement(item.element || "div");
        !0 === parent &&
          (!selectedExists &&
            idx <= 0 &&
          ...
          item.children &&
            ...
            setItems(el, item.children, !1)),
          item.class &&
            ("string" != typeof item.class &&
              console.warn("Please ensure item.class is a string"),
            (el.className += item.class)),
          item.style &&
            ("string" != typeof item.style &&
              console.warn("Please ensure item.style is a string"),
            (el.style.cssText = item.style)),
          item.text &&
            ("string" != typeof item.text &&
              console.warn("Please ensure item.text is a string"),
...
function setMenu(destEl, items, parent) {
  if (Array.isArray(items)) {
    for (var selectedExists, i = 0; items.length > i; i++)
      items[i].hasOwnProperty("uid") || getUID(items[i]);
    if (
    ...
    destEl.childNodes.forEach(function (node) {
      if (node.className)
        return !!node.className.includes(selectedList) && (selectedExists = !0);
    }),
      items.forEach(function (item, idx) {
        var textTitle = document.createTextNode(item.li || "item"),
...
function setItemListeners(destEl) {
  if (destEl instanceof HTMLElement) {
    if (
      (destEl.childNodes && (destEl = destEl.childNodes),
      !(destEl instanceof NodeList))
    )
      return console.warn("Warning Element provided does not have a nodelist!");
    destEl.forEach(function (el) {
      (el.className && !el.className.includes(itemCls)) ||
        (el.addEventListener("mouseenter", onEnter, !1),
        el.addEventListener("mouseleave", onLeave, !1),
        el.addEventListener("click", onClick, !1));
    });
  }
}
...
function setListListeners(destEl) {
  if (destEl instanceof HTMLElement) {
    if (
      ...
      return console.warn("Warning Element provided does not have a nodelist!");
    destEl.forEach(function (el) {
      (el.className && !el.className.includes(listCls)) ||
        el.addEventListener("click", onClick, !1);
    });
  }
}
